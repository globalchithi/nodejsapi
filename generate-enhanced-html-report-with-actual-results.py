...
def main():
    ...
    print("🔬 Generating enhanced HTML report with actual results...")
    
    # Check if TRX file exists
    if not os.path.exists(args.trx):
        print(f"❌ TRX file not found: {args.trx}")
        sys.exit(1)
    ...
    if generate_html_report(data, html_report_path):
        print("🎉 Enhanced HTML report with actual results generation completed!")
    else:
        sys.exit(1)
